...
% links
    ...
    for idx = 1:numel(J)    
        ...
        J{idx} = arm_Jacobian(link_vectors,joint_angles,joint_axes,idx);
    ...
    n = m;
    fignum = 316;
    ax = create_subaxes(fignum, m, n, p);
...
for idx = 1:numel(ax)
    ...
    % q
for idx = 1:numel(ax)
        q{idx} = draw_vectors_at_point(link_end_set{idx},J{idx}, ax{idx});
end
    ...
    % corresponding to the columns of the Jacobian
      for idx = 1:numel(l2)      
        ...
        for idx2 = 1:idx
            r1 = link_end_set_with_base{idx2};
            r2 = link_end_set{idx};
            X = [r1, r2];
            color = get(q{idx}{idx2}, 'Color');
            l2{idx}{idx2} = line(ax{idx}, X(1,:), X(2,:), X(3,:), 'Color', color, 'LineStyle', ':');
        end
        ...
        %   [Use get(q{idx}{idx2},'Color') to get the color of the arrow]
    for idx2 = 1:idx
    x{idx} = [link_ends(:,idx2), link_ends(:,idx2)+joint_axis_vectors_R{idx2}];
    color = get(q{idx}{idx2},'Color');
    l3{idx}{idx2} = line(ax{idx}, x{idx}(1,:), x{idx}(2,:), x{idx}(3,:), 'Color', color, 'LineStyle', '--');
