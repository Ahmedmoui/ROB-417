...
%       Euler-Lagrange equation
    ...
    configuration = zeros(numel(state)/2, 1);
    for idx = 1:numel(state)/2
        configuration(idx) = state(idx);
    end
    
    velocity = zeros(numel(state)/2, 1);
    for idx = 1:numel(velocity)
        velocity(idx) = state(idx + numel(velocity));
    end
    ...
    acceleration = EulerLagrange_acceleration(M_function, dM_function, F_function, time, configuration, velocity);
